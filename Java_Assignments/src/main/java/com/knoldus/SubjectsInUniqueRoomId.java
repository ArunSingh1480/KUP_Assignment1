...
public class SubjectsInUniqueRoomId {

    public String Condition2() {
        List<List<Student>> t = Data_All.ClassList().stream().filter(obj -> obj.getRoomId().equals("xyz")).map(k -> k.getStudentList().get()).collect(Collectors.toList());
